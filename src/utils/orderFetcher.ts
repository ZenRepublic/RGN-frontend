...
async function fetchOrdersFromEndpoint({
  cacheKey,
  endpoint,
  params,
...
export async function fetchOrderById(orderId: string, useCache:boolean=true): Promise<Order | null> {
  ...
  try {
    const response = await fetch(`${API_URL}/rgn/orders/${orderId}`, {
      method: 'GET',
      headers: {
        'Content-Type': 'application/json',
      },
    ...
    const order: Order = { ...raw, id: raw.id || raw._id || '' };

    if (order && order.id) {
      cacheOrders([order]);
      queryCache.set(cacheKey, [order.id]);
      return order;
    }
